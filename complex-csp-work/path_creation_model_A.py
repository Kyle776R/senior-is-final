...
# PATH CREATION MODEL -- PICKUP PLACEMENT VARIES  #
# Black pickups cannot be moved; randomly placed  #
# Blue pickups can be moved if their score is >0  #
# Blue pickups turn purple when they are placed   #

# ** Expected behavior BEGINS **
#
# Unmovable Pickup = Black = Initial score of -4
# Movable Pickup = Blue = Initial score of -2
#
# 1 movable pickup can attach to any side of an unmovable pickup, up to all 4 directions
# Movable pickups can be placed N/S/E/W of other movable pickups, but only 2 at a time
#
# ** Expected behavior ENDS **
...
agents_window = GraphWin('Pathfinding Model', WIN_WIDTH, WIN_HEIGHT)
    ...
    for _ in range(NUM_OF_UNMOVABLE_PICKUPS):
        ...
        while overlapping_pickups is True:
            ...
            for pickup in list_of_pickups:
                if (pickup.getCenter().getX() - (PICKUP_HEIGHT * 2) < center.getX() <
                    pickup.getCenter().getX() + (PICKUP_HEIGHT * 2)) and \
                        (pickup.getCenter().getY() - (PICKUP_HEIGHT * 2) < center.getY() <
                         pickup.getCenter().getY() + (PICKUP_HEIGHT * 2)):
                    overlapping_pickups = True
        ...
        pickup_to_add.draw(agents_window)
...
def create_agents():
    ...
    for i in range(num_agents_per_row):
        for j in range(num_agents_per_row):
            ...
            agent_to_add.draw(agents_window)
...
def shortcut_technique(agent, nearby_pickup_0, nearby_pickup_1, agents_list, pickups_list,
    ...
    global PLACED_PICKUP_POINTS
    #### print(f"allow_shortcut: {allow_shortcut}; indices: {nearby_pickup_indices}")
    ...
    #### print(f"({agent_0_x1}, {agent_0_y1}) and ({agent_0_x2}, {agent_0_y2})")
    #### print(f"vs. ({agent_1_x1}, {agent_1_y1}) and ({agent_1_x2}, {agent_1_y2})")
    ...
    if no_overlap is True:
        ...
        moved_pickup.draw(agents_window)
                            ...
                            if all_statuses_set is True:
                                ...
                                if num_choices >= 2:
                                    if agent_0_is_north is True and nearby_pickup_0.north_open is True:
                                        ...
                                        #### print(f"2 NUM_CHOICES: {num_choices}")
                                ...
                                if num_choices >= 2:
                                    if agent_1_is_north is True and nearby_pickup_1.north_open is True:
                                        ...
                                        #### print(f"3 NUM_CHOICES: {num_choices}")
                        ...
                        if case_number == 1:
                            ...
                            if all_statuses_set is True:
                                ...
                                if num_choices >= 2:
                                    ...
                                    # else:
                                        #### print(f"1 NUM_CHOICES: {num_choices}")
                        ...
                        if pickup_was_placed is True:
                            for placed_pickup in placed_pickups_list:
                                ...
                                if total_number_true >= 2:
                                    # print(f"~~ total_number_true: {total_number_true} ~~")
                                    # print(f"N: {pickup_to_north} S: {pickup_to_south} E: {pickup_to_east} W: {pickup_to_west}")
                                    # agents_window.getMouse()
                                    ...
                                    if len(set(list_of_line_ids)) != 1:
                                        ...
                                        # placed_pickup.setFill(color_rgb(128, 128, 128))
                                        # print("This counts as a connection between 2 paths.")
                                        # agents_window.getMouse()
            ...
            if num_agents_holding == 0:
                ...
                if len(num_iterations_per_path) > 0:
                    for num_iterations in num_iterations_per_path:
                        print(f"Path #{index} was created in {num_iterations} iterations.")
                        index += 1
                ...
                print(f"Length of pickup_list before: {len(pickups_list)}")
                ...
                # We need to update the unmovables that have permanents attached to them to ensure that
                # agents won't be attracted to spots that are clearly already occupied.
                for pickup in pickups_list:
                    if pickup.identity == "permanent" or pickup.on_path_id == "CONNECTED":
                        for unmovable_index in list_of_unmovable_indices:
                            unmovable = pickups_list[unmovable_index]

                            # The connecting movable is to the north of the unmovable.
                            if pickup.getCenter().getX() == unmovable.getCenter().getX() and \
                                    pickup.getCenter().getY() + PICKUP_HEIGHT == unmovable.getCenter().getY() and \
                                    unmovable.north_open is True:
                                unmovable.north_open = False
                                unmovable.score += 1

                            # The connecting movable is to the south of the unmovable.
                            if pickup.getCenter().getX() == unmovable.getCenter().getX() and \
                                    pickup.getCenter().getY() - PICKUP_HEIGHT == unmovable.getCenter().getY() and \
                                    unmovable.south_open is True:
                                unmovable.south_open = False
                                unmovable.score += 1

                            # The connecting movable is to the east of the unmovable.
                            if pickup.getCenter().getY() == unmovable.getCenter().getY() and \
                                    pickup.getCenter().getX() - PICKUP_HEIGHT == unmovable.getCenter().getX() and \
                                    unmovable.east_open is True:
                                unmovable.east_open = False
                                unmovable.score += 1

                            # The connecting movable is to the west of the unmovable.
                            if pickup.getCenter().getY() == unmovable.getCenter().getY() and \
                                    pickup.getCenter().getX() + PICKUP_HEIGHT == unmovable.getCenter().getX() and \
                                    unmovable.west_open is True:
                                unmovable.west_open = False
                                unmovable.score += 1

                for remove_pickup in pickups_to_remove:
                    pickups_list.remove(remove_pickup)

                agents_window.getMouse()

                # Place new movables and then randomize their locations.
                PLACED_PICKUP_POINTS = []

                # If there are no more movables to re-add, then the program is finished.
                if num_remaining_pickups == 0:
                ...
                print(f"Length of pickup_list after: {len(pickups_list)}")

                # for pickup in pickups_list:
                #     if pickup.identity == "unmovable":
                #         print(f"unmovable at location {pickup.getCenter()} has:")
                #         print(f"N: {pickup.north_open} || S: {pickup.south_open} || "
                #               f"E: {pickup.east_open} || W: {pickup.west_open}")
...
def main():
    ...
    # After the agents are finished, the user clicks again to exit the program.
    print("The agents have finished making paths. Click the screen again to exit.")
    agents_window.getMouse()
    agents_window.close()
