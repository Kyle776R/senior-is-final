...
# DATA COLLECTION CODE FOR ALGORITHM #2 (PICKUPS PLACED ARE NOT PLACED PERMANENTLY) #

#
#
# ** All global variables in this file are set using the function reset_global_variables()! **
#
#
...
def agent_animation(agents_list, pickups_list, x_low, x_high, y_low, y_high):
    ...
    while continue_experiment:
            ...
            if agent.is_holding is True:
                ...
                # Determines if an agent should drop a pickup, if it is holding one.
                    ...
                    if drop_pickup is True:
                        moved_pickup = Pickup(agent.getP1(), agent.getP2())
                        moved_pickup.setFill(color_rgb(0, 0, 255))  # Blue
                        moved_pickup.setOutline(color_rgb(0, 0, 0))  # Black

                        # moved_pickup.draw(agents_window)  # [1] data collection
        ...
        agents_still_holding = False
        num_successful_placements = 0

        for pickup in pickups_list:
            increment_counter = False

            for pickup_to_compare in pickups_list:
                if pickup == pickup_to_compare:
                    increment_counter = True
                else:
                    distance_squared = ((pickup.getCenter().getX() - pickup_to_compare.getCenter().getX()) ** 2) + \
                                       ((pickup.getCenter().getY() - pickup_to_compare.getCenter().getY()) ** 2)
        ...
        for agent in agents_list:
            if agent.is_holding is True:
                agents_still_holding = True
                break

        if num_successful_placements == NUM_OF_PICKUPS and agents_still_holding is False:
...
def reset_global_variables():
    global WIN_WIDTH, WIN_HEIGHT, NUM_OF_ITERATIONS, NUM_OF_AGENTS, NUM_OF_PICKUPS, AGENT_RADIUS, AGENT_VELOCITY, \
        TIME_BETWEEN_SET_DOWNS, TIME_TO_NOT_PICK_UP, FIND_PICKUPS_RANGE, NO_SPAWN_RANGE, PICKUP_HEIGHT, \
        DIST_BETWEEN_PICKUPS, GRABBED_OBJECT_LIST
...
def main():
    num_times_to_loop = 100

    for _ in range(num_times_to_loop):
